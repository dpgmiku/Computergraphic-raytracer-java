...
//
    ...
    public static void main(String[] args) throws IOException {
        ...
        Thread thread1 = new WorkingThread(0, kamera.HEIGHT / 4, kamera, shapes, scanrate, SCENE);
        Thread thread2 = new WorkingThread(kamera.HEIGHT / 4, kamera.HEIGHT - kamera.HEIGHT / 2, kamera, shapes, scanrate, SCENE);
        Thread thread3 = new WorkingThread(kamera.HEIGHT - kamera.HEIGHT / 2, kamera.HEIGHT - kamera.HEIGHT / 4, kamera, shapes, scanrate, SCENE);
        Thread thread4 = new WorkingThread(kamera.HEIGHT - kamera.HEIGHT / 4, kamera.HEIGHT, kamera, shapes, scanrate, SCENE);

        WorkingThread[] threads = {thread1, thread2, thread3, thread4};
        ...
        HashMap<int[], Vec3> map = new HashMap<>();

        for (WorkingThread t : threads) {
            map.putAll(t.map);
        }

        for (Map.Entry<int[], Vec3> entry : map.entrySet()) {
            int[] key = entry.getKey();
            Vec3 value = entry.getValue();
            image.setPixel(key[0], key[1], value);
        }
